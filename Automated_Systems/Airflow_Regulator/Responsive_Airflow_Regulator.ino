...
void loop() {
  ...
  // Fan control logic based on humidity
  if (calibratedHumidity > 90) {
    digitalWrite(fan, HIGH);  // Turn the fan ON
    Serial.println("Fan activated: Humidity exceeded 90%");
  } 
  else if (calibratedHumidity < 70) {
    digitalWrite(fan, LOW);  // Turn the fan OFF
    Serial.println("Fan deactivated: Humidity dropped below 70%");
  }
